...
char inp[100005];
int f[200005];
int failure_func(int len)
{
    int pt = 0;
    f[0]=0;
    for(int i = 1; i<len;)
    {
        if(str[i]==str[pt]) f[i]=++pt,i++;
...
int main()
{
    ...
    while(sf("%s",inp)==1)
    {
        ...
        int j,len = strlen(inp);
        j = len-1;

        REP(i,0,len) patern[i]=inp[j--];
        patern[len]='#';
        str[0] = 0;
        patern[len+1] = 0;
        strcat(str,patern);
        strcat(str,inp);
        // puts(str);
        int n = failure_func(strlen(str));

       // pf("%d\n",len-n+len);
        REP(i,0,len) pf("%c",inp[i]);
        for(int i = len-n-1; i>=0; i--) pf("%c",inp[i]);
        puts("");
